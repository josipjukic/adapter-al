...
if __name__ == "__main__":
    ...
    for sampler_name in args.al_samplers:
        ...
        for i, seed in zip(range(1, args.repeat + 1), seeds):
            ...
            sampler_names = ["entropy", "core_set", "dal", "repr"]
            sampler_cls = [get_al_sampler(name) for name in sampler_names]
            samplers = [sc(
                dataset=train,
                batch_size=args.batch_size,
                device=device,
            ) for sc in sampler_cls]
            active_learner = Experiment(samplers, train, test, device, args, meta)

            results = active_learner.besov_al(
