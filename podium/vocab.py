...
class Vocab:
    ...
    def __init__(
        ...
        if len(self.specials) > len(set(self.specials)):
            raise ValueError(
                "Specials may not contain multiple instances of same type."
            )
    ...
    def numericalize(self, data: Union[str, Iterable[str]]) -> np.ndarray:
        ...
        if Vocab._unk in self.stoi:
            ...
            return np.array(
                [
                    self.stoi[token] if token in self.stoi else unk_token
                    for token in data
                ]
            )
        else:
    ...
    def __hash__(self) -> int:
        return hash(
            (self.is_finalized, self._freqs, self.stoi, self.itos, self.specials)
        )
