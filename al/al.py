...
def get_unlabeled_idx(X_train, labeled_idx):
    ...
    return np.arange(X_train.shape[0])[
        np.logical_not(np.in1d(np.arange(X_train.shape[0]), labeled_idx))
    ]
...
class DiscriminativeSampling(QueryMethod):
    ...
    def query(self, X_train, Y_train, labeled_idx, amount):
        ...
        unlabeled_idx = np.random.choice(
            unlabeled_idx,
            np.min([labeled_idx.shape[0] * 10, unlabeled_idx.size]),
            replace=False,
        )
        ...
        while labeled_so_far < amount:
            ...
            model = train_discriminative_model(
                X_train[labeled_idx],
                X_train[unlabeled_idx],
                self.input_shape,
                gpu=self.gpu,
            )
            predictions = model.predict(X_train[unlabeled_idx])
            selected_indices = np.argpartition(predictions[:, 1], -sub_sample_size)[
                -sub_sample_size:
            ]
...
class DiscriminativeRepresentationSampling(QueryMethod):
    ...
    def query(self, X_train, Y_train, labeled_idx, amount):
        ...
        representation = embedding_model.predict(X_train, batch_size=128).reshape(
            (X_train.shape[0], -1, 1)
        )
    ...
    def query(self, X_train, Y_train, labeled_idx, amount):
        ...
        if self.autoencoder is None:
            ...
            self.autoencoder.compile(
                optimizer=optimizers.Adam(lr=0.0003), loss="binary_crossentropy"
            )
            self.autoencoder.fit(
                X_train, X_train, epochs=200, batch_size=256, shuffle=True, verbose=2
            )
            encoder = Model(
                self.autoencoder.input, self.autoencoder.get_layer("embedding").input
            )
        ...
        while labeled_so_far < amount:
            ...
            model = train_discriminative_model(
                self.embedding[labeled_idx],
                self.embedding[unlabeled_idx],
                self.embedding[0].shape,
                gpu=self.gpu,
            )
...
class DiscriminativeStochasticSampling(QueryMethod):
    ...
    def query(self, X_train, Y_train, labeled_idx, amount):
        ...
        representation = embedding_model.predict(X_train, batch_size=256).reshape(
            (X_train.shape[0], -1, 1)
        )
        ...
        while labeled_so_far < amount:
            ...
            selected_indices = np.random.choice(
                unlabeled_idx, sub_sample_size, replace=False, p=predictions[:, 1]
            )
...
class CoreSetSampling(QueryMethod):
    ...
    def greedy_k_center(self, labeled, unlabeled, amount):
        ...
        min_dist = np.min(
            distance_matrix(labeled[0, :].reshape((1, labeled.shape[1])), unlabeled),
            axis=0,
        )
...
class CoreSetMIPSampling(QueryMethod):
        ...
        for i in range(0, representation.shape[0], 1000):
            ...
            if i + 1000 > representation.shape[0]:
                ...
                distances = self.get_distance_matrix(
                    representation[i : i + 1000], representation
                )
                amount = 1000
            ...
            for j in range(i, i + amount):
                graph[j] = [
                    (idx, distances[j - i, idx])
                    for idx in np.reshape(np.where(distances[j - i, :] <= delta), (-1))
                ]
    ...
    def mip_model(
        ...
        for i in range(representation.shape[0]):
            if i in labeled_idx:
                points[i] = model.addVar(
                    ub=1.0, lb=1.0, vtype="B", name="points_{}".format(i)
                )
            else:
        ...
        model.addConstr(
            sum(points[i] for i in range(representation.shape[0])) == budget, "budget"
        )
        neighbors = {}
        graph = {}
    ...
    def mip_model_subsample(
        ...
        model.addConstr(
            sum(points[i] for i in range(data.shape[0])) == budget, "budget"
        )
        neighbors = {}
        for i in range(data.shape[0]):
            neighbors[i] = []
    ...
    def query_regular(self, X_train, Y_train, labeled_idx, amount):
        ...
        representation = representation_model.predict(
            X_train, batch_size=128, verbose=0
        )
        print("Calculating Greedy K-Center Solution...")
        new_indices, max_delta = self.greedy_k_center(
            representation[labeled_idx], representation[unlabeled_idx], amount
        )
        ...
        model, graph = self.mip_model(
            representation,
            labeled_idx,
            len(labeled_idx) + amount,
            upper_bound,
            outlier_count,
            greedy_indices=new_indices,
        )
        ...
        while upper_bound - lower_bound > eps:
            ...
            print(
                "upper bound is {ub}, lower bound is {lb}".format(
                    ub=upper_bound, lb=lower_bound
                )
            )
            if model.getAttr(gurobi.GRB.Attr.Status) in [
                gurobi.GRB.INFEASIBLE,
                gurobi.GRB.TIME_LIMIT,
            ]:
                print("Optimization Failed - Infeasible!")

                lower_bound = max(
                    current_delta, self.get_graph_min(representation, current_delta)
                )
                ...
                upper_bound = min(
                    current_delta, self.get_graph_max(representation, current_delta)
                )
                current_delta = (upper_bound + lower_bound) / 2.0
                indices = [i for i in graph if points[i].X == 1]
...
class EGLSampling(QueryMethod):
    ...
    def compute_egls(self, unlabeled, n_classes):
        ...
        self.get_gradient_length = K.function(
            [K.learning_phase(), self.input_placeholder, self.output_placeholder],
            [gradient_length],
        )
        ...
        for i in range(n_classes):
            ...
            while calculated_so_far < unlabeled_predictions.shape[0]:
                ...
                grads = self.get_gradient_length(
                    [
                        0,
                        unlabeled[calculated_so_far : calculated_so_far + next, :],
                        labels,
                    ]
                )[0]
                grads *= unlabeled_predictions[
                    calculated_so_far : calculated_so_far + next, i
                ]
