...
class ExactJointEntropy(JointEntropy):
    ...
    def compute(self) -> torch.Tensor:
        probs_M = torch.mean(self.joint_probs, dim=1, keepdim=False)
        nats_M = -torch.log(probs_M) * probs_M
        entropy = torch.sum(nats_M)
        return entropy

    def add_variables(self, log_probs: torch.Tensor) -> "ExactJointEntropy":
        assert self.joint_probs.shape[1] == log_probs.shape[1]

        N, K, C = log_probs.shape
        joint_probs_K_M_1 = self.joint_probs.t()[:, :, None]

        probs_N_K_C = log_probs.exp()
        ...
        self.joint_probs = joint_probs_K_M_1.squeeze(2).t()
        return self

    def compute_batch(self, log_probs, output_entropies_B):
        assert self.joint_probs.shape[1] == log_probs.shape[1]

        B, K, C = log_probs.shape
        M = self.joint_probs.shape[0]

        if output_entropies_B is None:
            output_entropies_B = torch.empty(
                B, dtype=log_probs.dtype, device=log_probs.device
            )

            probs_B_K_C = log_probs.exp()
            b = probs_B_K_C.shape[0]

            probs_B_M_C = torch.empty(
                (b, M, C),
                dtype=self.joint_probs.dtype,
                device=self.joint_probs.device,
            )
            for i in range(b):
                torch.matmul(
                    self.joint_probs,
                    probs_B_K_C[i].to(self.joint_probs, non_blocking=True),
                    out=probs_B_M_C[i],
                )
            probs_B_M_C /= K
