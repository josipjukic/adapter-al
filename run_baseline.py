...
if __name__ == "__main__":
    ...
    for sampler_name in args.al_samplers:
        ...
        for i, seed in zip(range(1, args.repeat + 1), seeds):
            ...
            sampler_cls = get_al_sampler("random")
            sampler = sampler_cls(
                dataset=train, batch_size=args.batch_size, device=device
            )
            active_learner = Experiment(sampler, train, test, device, args, meta)

            results = active_learner.baseline_loop(
        ...
        fname = (
            f"{args.data}-{args.model}-baseline-"
