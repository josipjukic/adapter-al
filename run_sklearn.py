...
if __name__ == "__main__":
    ...
    (train, val, test), vocab = dataloader(meta=meta)
    ...
    for sampler_name in args.al_samplers:
        ...
        # logging.info(f"{train[:10].examples}")
        ...
        for i, seed in zip(range(1, args.repeat + 1), seeds):
            ...
            active_learner = Experiment(sampler, train, val, device, args, meta)

            results = active_learner.sklearn_al_loop(
        ...
        fname = f"{args.data}-{args.model}-{sampler.name}-all-{start_time}.pkl"
